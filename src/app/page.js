...
export default function Home() {
  ...
  const fetchJoinEvents = async (meetingId) => {
    try {
      const response = await axios.get(
        `${API_LINK}/api/meetings/${meetingId}/participants/joined-people`,
        {
          params: {
            access_token: accessToken,
          },
  ...
  const fetchLeaveEvents = async (meetingId) => {
    try {
      const response = await axios.get(
        `${API_LINK}/api/meetings/${meetingId}/participants/left`,
        {
          params: {
            access_token: accessToken,
          },
    ...
    <div style={styles.container}>
      ...
      >
        ...
        <div style={styles.formGroup}>
          <label style={styles.label}>
            Meeting Invitees (comma-separated emails):
          </label>
          <input
            style={styles.input}
            type="text"
      ...
      {meetingData && (
        <div style={styles.meetingDetails}>
          ...
          <button
            style={styles.button}
            onClick={() => fetchJoinEvents(meetingData.id)}
          >
            Fetch Join Events
          </button>
          <button
            style={styles.button}
            onClick={() => fetchLeaveEvents(meetingData.id)}
          >
            Fetch Leave Events
          </button>
          ...
          <ul style={styles.list}>
            {joinEvents.map((event, index) => (
              <li key={index}>
                Name: {event.name}, Join Time: {event.joinedAt}
              </li>
            ))}
          </ul>
          ...
          <ul style={styles.list}>
            {leaveEvents.map((event, index) => (
              <li key={index}>
                Name: {event.name}, Leave Time: {event.leave_time}
              </li>
            ))}
          </ul>
        </div>
